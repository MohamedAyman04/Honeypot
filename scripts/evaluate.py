...
def plot_thesis_visuals(df, phases, if_thresh, lstm_thresh, fusion_thresh):
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "thesis_timeline.png", dpi=200)
    print(f"[PLT] Saved: {FIGURES_DIR / 'thesis_timeline.png'}")
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "thesis_confusion_matrices.png", dpi=200)
    print(f"[PLT] Saved: {FIGURES_DIR / 'thesis_confusion_matrices.png'}")
    ...
    if not f1_df.empty:
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig(FIGURES_DIR / "thesis_f1_protocol.png", dpi=200)
        print(f"[PLT] Saved: {FIGURES_DIR / 'thesis_f1_protocol.png'}")
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "thesis_pr_curves.png", dpi=200)
    print(f"[PLT] Saved: {FIGURES_DIR / 'thesis_pr_curves.png'}")
...
"""
    ...
    with open(REPORTS_DIR / "evaluation_report.txt", "w") as f:
        f.write(report)
    print(f"[RPT] Saved: {REPORTS_DIR / 'evaluation_report.txt'}")
...
def main():
    ...
    print(f"\n[SUCCESS] Evaluation complete. Artifacts saved in figures/ and reports/.")
